<template>
  ...
  <div id="page-organization-list" class="wrapper wrapper-content">
    <h1 v-if="!tenantAdmin"><img src="../../assets/imgs/permisson.png"></h1>
        ...
        <div class="col-sm-4">
          <div class="form-group align-right" style="margin-top:23px;">
            <div class="row">
              ...
              <div class="col-md-6">
                <el-button type="light" @click="create">新建用户</el-button>
              </div>
            </div>
          </div>
        </div>
        ...
        <el-table
          ...
          <el-table-column prop="name" label="用户名称" width="200">
            <template slot-scope="scope">
              <a @click="go_detail(scope.row)">{{scope.row.loginName}}</a>
            </template>
          </el-table-column>
          <el-table-column  prop="isTenantAdmin" label="是否管理员" align="center"></el-table-column>
          <el-table-column  prop="disabled" label="是否禁用" align="center"></el-table-column>
          <el-table-column  prop="tenant" label="所属租户" align="center"></el-table-column>
...
export default {
  ...
  methods: {
    ...mapActions(['USER_LIST', 'USER_DELETE', 'USER_POWER']),
    search () {
      this.fetch()
    },
    searchByState () {},
    ...
    fetch () {
      this.loading = true
      this.tenantAdmin = this.user.isTenantAdmin || this.user.isAdmin
      this.$store.dispatch('USER_LIST', this.searchDatas)
        .then(res => {
          if (res.success) {
    ...
    parseData (datas) {
      each(datas, item => {
        ...
        item.isTenantAdmin = item.isTenantAdmin ? '是' : '否'
      })
    },
    stop (row) {
      let str = row.disabled === '是' ? '启用' : '停用'
      this.$confirm(`确定要${str} ”${row.loginName}” ？`, '操作提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消'
      }).then(() => {
        row.disabled = row.disabled === '是'
        row.isTenantAdmin = row.isTenantAdmin === '是'
        this.$store.dispatch('USER_POWER', row._id)
          .then(res => {
            if (res.success) {
    ...
    remove (row) {
      this.$confirm(`确定要删除 ”${row.loginName}” ？`, '操作提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消'
      }).then(() => {
        this.$store.dispatch('USER_DELETE', row._id)
          .then(res => {
            if (res.success) {
              this.$resetPage(this.table)
              this.fetch()
