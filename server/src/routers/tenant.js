...
export const detailSave = conext (async (req, res) => {
  ...
  if (!newTenant) {
    ...
    for (let i = 0; i < oldTenant.member.length; i++) {
      ...
      let a = await User.update(user, {tenant: post.name})
    }
    // 修改名称成功，相关系统也需修改租户名
    let buckets = Bucket.getByQuery({tenant: oldTenant.name})
    for (let i = 0; i < buckets.length; i++) {
      let a = await Bucket.update(buckets[i], {tenant: post.name})
    }
    // 修改名称成功，相关api也需修改租户名
    let apis = Api.getByQuery({tenant: oldTenant.name})
    for (let i = 0; i < apis.length; i++) {
      let a = await Api.update(apis[i], {tenant: post.name})
    }
    let r = await Tenant.update({_id: post.id, name: post.name})
    change = true
  ...
  if (!user || (!change && user.tenant != oldTenant.name) || (change && user.tenant != post.name))  {
    ...
    let b = await User.update({_id: post.adminId}, {isTenantAdmin: false})
...
export const disable = conext (async (req, res) => {
  ...
  for (let i = 0; i < tenant.member.length; i++) {
    ...
    let a = await User.disable(tenant.member[i].id, user.disabled)
  }

  // 改变下属系统
  let buckets = await Bucket.getByQuery({tenant: tenant.name})
  for (let i = 0; i < buckets.length; i++) {
    let b = await Bucket.disable(buckets[i]._id, buckets[i].disabled)
  }
  // 改变下属api
  let apis = await Api.getByQuery({tenant: tenant.name})
  for (let i = 0; i < apis.length; i++) {
    let c = await Api.disable(apis[i]._id, apis[i].disabled)
  }
  // 禁用租户
